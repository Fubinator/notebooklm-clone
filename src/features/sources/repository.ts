  ...
  return {
    async list(notebookId) {
      const { data, error } = await supabase
        .from("sources")
        .select(
          "id, notebook_id, title, kind, original_url, attribution, license_name, license_url, content, processing_stage, embedding_provider, embedding_model, embedding_dimensions, embedding_pooling, character_count, failure_category, retry_stage, attempt_count, correlation_id, created_at, updated_at, passages(id, source_id, ordinal, content, page_number, paragraph_start, paragraph_end, created_at)",
        )
